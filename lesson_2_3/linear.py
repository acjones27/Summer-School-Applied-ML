...
def feat_graph(df,icol,binary_col,n_buckets):
	...
	# cut data into buckets so that each bucket has the same number of observations
...
# based on graphs, decide which ones should be imputed with min and which with max
Impute_min=['X27', 'X28', 'X37', 'X45', 'X53', 'X54', 'X60', 'X64']
Impute_max=['X24', 'X41']
# For the X41, you could put the median. but if you're using trees, min/max works better
...
# Impute
# this is calling the fancyimpute library
from sklearn.experimental import enable_iterative_imputer
from sklearn.impute import IterativeImputer

filler=IterativeImputer()
# fit and transform at once. can separate if you want
X_train_filled = filler.fit_transform(X_train)
# only transform based on the one trained on the training data
X_test_filled = filler.transform(X_test)

# can't use knn from fancyimpute because only have fit_transform, not transform alone. would be nice to fix this and make a PR. what language is it in? --> CHECK
...
def InspectCorrelated(df):
	...
	# gives a matrix of correlations
...
GINIS=list()
ACTIVATIONS=list()
# TMR - Relative default rate
# ODF - Observed default ... Factor?
TMRS=list()
...
for FACTOR in list(train)[len(Categorical):-1]:
	...
	# GINI again to be sure that all are positive
	A_train=alerts_train[FACTOR+'_b']
	gini=roc_auc_score(Y,A_train)*2-1
	...
	# TMR
	# TAF - Taza de activacion de flag??
...
# For linear models
# Every variable is converted into a WoE variable
...
# By default, penalty = 'l1' ie lasso and C=1
MODEL=LogisticRegression()
MODEL.set_params(**params)

# We will define the model for various C's and make a search of the optimum
# Inverse of lamba which is the absolute value of the betas for the 'l1' regularization
...
for c in Cs:
	...
	# it outputs the prob of 0 and prob of 1. we select the second column as we want the prob of 1
...
print('\n###########################################')
print('Lasso optimal C: ',best_C)
print('Lasso optimal GINI: ',round((max(scores)*2-1)*100,4),'%')
print('Lasso optimal AUC: ',round(max(scores)*100,4),'%')
print('###########################################')

# When we've picked the best C, we should run the train test on the whole training set against our hold-out test set
# train a model on all the whole train with the optimal parameter C:
model_lasso=LogisticRegression()
model_lasso.set_params(**params)
model_lasso.set_params(C=best_C)

model_lasso.fit(X_train_woe, Y_train)
weights = pd.DataFrame({'feature':np.array(X_train_woe.columns),'coefs':np.transpose(np.array(model_lasso.coef_))[:,0]})


################################################################################
# Results
# Prediction
test['Pred']=model_lasso.predict_proba(X_test_woe)[:,1]
outputs_lasso=pd.DataFrame(test[['ID','Pred']])

# Outputs to .csv
outputs_lasso.to_csv("output/outputs_lasso.csv", index = False)
weights.to_csv("output/lasso_features.csv", index = False)
################################################################################
...
# later, we see that if makepred == True --> take the predictions as a new feature i.e. modelos de segundo nivel
# If we do a catboost, we need to tell the function where the categorical vars are
def Model_cv(MODEL, k, X_train, X_test, y, RE, makepred=True, CatPos=None):
	# Create the k folds
	# Stratified tries to have the same proportion of 1/0s in train/test
	...
	for train_index, test_index in kf.split(X_train, Y_train):
		...
		print(k, '-cv, Fold ', count, '\t --test GINI: ', round(score*2-1,4), '\t--train GINI: ', round(score_train*2-1,4),sep='')
	...
	print('\n',k, '- cv, TOTAL GINI:', round((score_total*2-1)*100,4),'%')
...
model_lasso.set_params(**params)
model_lasso.set_params(C=best_C)
# model_lasso.set_params(penalty='l2')

model_lasso.fit(X_train_woe, Y_train)
weights = pd.DataFrame({'feature':np.array(X_train_woe.columns),'coefs':np.transpose(np.array(model_lasso.coef_))[:,0]})
# if we view the weights, we can see that a lot of features are 0 because we do l1. This can be used for feature selection. if we change the penalty to l2, nothing will be 0 but there'll be some reduceed

# RESULTS
# Prediction
# careful: here we use test_woe since we used train_woe
test['Pred']=model_lasso.predict_proba(X_test_woe)[:,1]
outputs_lasso=pd.DataFrame(test[['ID','Pred']])

# Outputs to .csv
# we would upload this into kaggle
outputs_lasso.to_csv("output/kfold_outputs_lasso.csv", index = False)
weights.to_csv("output/kfold_lasso_features.csv", index = False)
