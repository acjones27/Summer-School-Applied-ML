...
train = pd.DataFrame(my_imputer.fit_transform(train.iloc[:,:-1]))
...
# 													 verbose=False)
#
# ShapValues=pd.DataFrame(ShapImportance.iloc[:,:-1], columns=list(X_train))
#
...
# ShapValues['EXP_VAL_LO'] = ShapImportance.iloc[0,-1]
...
#
# ShapValues_p=pd.DataFrame()
# # COnversion factor approximation: shap_p(x_i) = x_i / SUMX_LO * SUMX_p:
# for col in cols:
#     ShapValues_p[col]=ShapValues[col]/ShapValues['SUMX_LO']*ShapValues['SUMX_p']
#
# # To ndarray
# ShapImportance_p = ShapValues_p.values
#
# # To see the data we need the Shap module
# # (installation: pip install Shap, with uppercase "S" but is called with "s")
# from shap import summary_plot
#
# # Case 1) LOG ODDS variation. Is not what we want but we can show it and explain it
# summary_plot(ShapImportance.iloc[:,:-1], X_train,max_display=20,plot_type='dot')
# # Case 2) Probability variation (The good to show)
# summary_plot(ShapImportance_p, X_train,max_display=20,plot_type='dot')
#
# # IMPORTANT: The sorting criteria of the feature importance is the sum of the absolute
# # values of the Shap values across all observations, that is, for the second case:
# Importance=ShapValues_p.abs().sum(axis=0).sort_values(ascending=False).reset_index(drop=False)
# Importance.columns = ['Feature', 'Imp']
...
# Outputs to .csv
outputs_catboost.to_csv("outputs_catboost_scaled_convertcatnum.csv", index = False)
